...
import { Download, UploadCloud, RotateCcw, RotateCw, FlipHorizontal, FlipVertical, Image as ImageIcon, Check, Layers, RefreshCw, Trash2, Plus, Bold, Italic, AlignLeft, AlignCenter, AlignRight } from 'lucide-react';
...
export default function Editor() {
  ...
  // Text form state (for editing selected or new text)
  ...
  const drawImage = useCallback(() => {
    ...
    img.onload = () => {
      ...
      ctx.drawImage(img, -dimensions.width / 2, -dimensions.height / 2, dimensions.width, dimensions.height);
      ctx.restore();
    };
    img.src = imageSrc;
  }, [imageSrc, dimensions, adjustments, transform, buildFilterString]);

  useEffect(() => {
    let raf: number;
    raf = requestAnimationFrame(drawImage);
    return () => cancelAnimationFrame(raf);
  }, [drawImage]);

  // Sync form state when selecting an annotation
  useEffect(() => {
    if (selectedId) {
  ...
  // Update selected annotation live as user edits form
  useEffect(() => {
  ...
  // Map canvas display rect coords to canvas pixel coords
  ...
  const handleDownload = () => {
    ...
    // Draw text annotations onto canvas before export
    ...
    canvas.toBlob((blob) => {
      ...
      // Redraw without text baked (restore canvas)
      drawImage();
  ...
  // Compute annotation position in canvas display space
  const getDisplayPos = (ann: TextAnnotation) => {
    ...
    return { left: ann.x * scaleX, top: ann.y * scaleY };
  };
      ...
      <div className="flex flex-1 overflow-hidden relative">
          ...
          >
            <canvas
              ...
              style={{
                ...
                cursor: textTool ? 'crosshair' : 'default',
              }}
            ...
            {annotations.map(ann => {
              ...
              return (
                <div
                  key={ann.id}
                  data-testid={`text-annotation-${ann.id}`}
                  style={{
                    position: 'absolute',
                    left: pos.left,
                    top: pos.top,
                    fontSize: `${ann.fontSize * (canvasRef.current ? canvasRef.current.getBoundingClientRect().width / canvasRef.current.width : 1)}px`,
        ...
        <aside className="w-80 border-l border-border bg-card flex flex-col shrink-0 z-10 shadow-2xl">
          <Tabs defaultValue="adjust" className="flex-1 flex flex-col">
            <TabsList className="w-full justify-start h-14 p-0 bg-transparent border-b border-border rounded-none shrink-0 flex">
              <TabsTrigger value="adjust" className="flex-1 rounded-none h-full text-xs data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:shadow-none" data-testid="tab-adjust">
                Adjust
              </TabsTrigger>
              <TabsTrigger value="filters" className="flex-1 rounded-none h-full text-xs data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:shadow-none" data-testid="tab-filters">
                Filters
              </TabsTrigger>
              <TabsTrigger value="text" className="flex-1 rounded-none h-full text-xs data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:shadow-none" data-testid="tab-text">
                Text
              </TabsTrigger>
              <TabsTrigger value="transform" className="flex-1 rounded-none h-full text-xs data-[state=active]:bg-background data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:shadow-none" data-testid="tab-layout">
                Layout
              </TabsTrigger>
            </TabsList>
              ...
              <TabsContent value="adjust" className="space-y-6 mt-0">
                ...
                <div className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <Slider value={[adjustments.sepia]} min={0} max={100} step={1} onValueChange={([v]) => setAdjustments(a => ({ ...a, sepia: v }))} data-testid="slider-sepia" />
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      <span>Hue</span>
                    ...
                    <Slider value={[adjustments.hueRotate]} min={0} max={360} step={1} onValueChange={([v]) => setAdjustments(a => ({ ...a, hueRotate: v }))} data-testid="slider-hue" />
                  </div>
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      <span>Blur</span>
                    ...
                    <Slider value={[adjustments.blur]} min={0} max={20} step={0.5} onValueChange={([v]) => setAdjustments(a => ({ ...a, blur: v }))} data-testid="slider-blur" />
                  </div>
                </div>
              </TabsContent>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {PRESET_COLORS.map(c => (
                          <button
                            ...
                            style={{
                              backgroundColor: c,
                              borderColor: textColor === c ? 'hsl(var(--primary))' : 'hsl(var(--border))',
                              boxShadow: textColor === c ? '0 0 0 2px hsl(var(--primary)/0.3)' : 'none',
                            }}
              ...
              <TabsContent value="transform" className="space-y-8 mt-0">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button variant="outline" className="h-12" onClick={() => setTransform(t => ({ ...t, flipH: !t.flipH }))} data-testid="button-flip-h">
                      <FlipHorizontal className="w-4 h-4 mr-2" /> Flip H
                    </Button>
                    <Button variant="outline" className="h-12" onClick={() => setTransform(t => ({ ...t, flipV: !t.flipV }))} data-testid="button-flip-v">
                      <FlipVertical className="w-4 h-4 mr-2" /> Flip V
                    </Button>
                    <Button variant="outline" className="h-12" onClick={() => setTransform(t => ({ ...t, rotate: t.rotate - 90 }))} data-testid="button-rotate-left">
                      <RotateCcw className="w-4 h-4 mr-2" /> Left
                    </Button>
                    <Button variant="outline" className="h-12" onClick={() => setTransform(t => ({ ...t, rotate: t.rotate + 90 }))} data-testid="button-rotate-right">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5 pt-2">
                      ...
                      <Select onValueChange={applyPresetFormat}>
                        <SelectTrigger className="h-9 bg-background" data-testid="select-preset-format">
