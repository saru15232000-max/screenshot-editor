...
import { Trash2, Plus, RefreshCw, Layers, Bold, Italic, AlignLeft, AlignCenter, AlignRight } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto p-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() => editor.addTextLayer(100, 100)}
                >
                  <Plus className="w-3 h-3" /> Add Text
                </Button>
              </div>
              {editor.annotations.filter(a => a.type === 'text').length === 0 && (
                <p className="text-xs text-muted-foreground py-3 text-center">No text layers yet. Click "Add Text" or use the Text tool on the canvas.</p>
              )}
              <div className="space-y-1">
                {editor.annotations.filter(a => a.type === 'text').map(ann => (
                  ...
                  >
                    ...
                    <button
                      onClick={e => { e.stopPropagation(); editor.pushHistory(editor.annotations.filter(a => a.id !== ann.id)); if (editor.selectedAnnotationId === ann.id) editor.setSelectedAnnotationId(null); }}
              ...
              <div className="space-y-4 pt-2 border-t border-border">
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={e => {
                      editor.pushHistory(editor.annotations.map(a =>
                        a.id === editor.selectedAnnotationId ? { ...a, text: e.target.value } : a
                      ));
                    }}
                    rows={3}
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={v => editor.pushHistory(editor.annotations.map(a =>
                      a.id === editor.selectedAnnotationId ? { ...a, fontFamily: v } : a
                    ))}
                  >
                    <SelectTrigger className="h-9 bg-background text-sm">
                      <SelectValue>
                        <span style={{ fontFamily: selectedAnnotation.fontFamily }}>
                          {FONT_FAMILIES.find(f => f.value === selectedAnnotation.fontFamily)?.label ?? 'Inter'}
                        </span>
                      </SelectValue>
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs text-muted-foreground">{selectedAnnotation.fontSize ?? 32}px</span>
                  </div>
                  <Slider
                    value={[selectedAnnotation.fontSize ?? 32]}
                    min={10} max={300} step={1}
                    onValueChange={([v]) => editor.pushHistory(editor.annotations.map(a =>
                      a.id === editor.selectedAnnotationId ? { ...a, fontSize: v, size: v } : a
                    ))}
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2">
                    {[
                      { icon: <Bold className="w-4 h-4" />,        active: !!selectedAnnotation.bold,                 key: 'bold',   update: { bold: !selectedAnnotation.bold } },
                      { icon: <Italic className="w-4 h-4" />,      active: !!selectedAnnotation.italic,               key: 'italic', update: { italic: !selectedAnnotation.italic } },
                      { icon: <AlignLeft className="w-4 h-4" />,   active: selectedAnnotation.align === 'left',       key: 'al',     update: { align: 'left' as const } },
                      { icon: <AlignCenter className="w-4 h-4" />, active: selectedAnnotation.align === 'center',     key: 'ac',     update: { align: 'center' as const } },
                      { icon: <AlignRight className="w-4 h-4" />,  active: selectedAnnotation.align === 'right',      key: 'ar',     update: { align: 'right' as const } },
                    ].map(({ icon, active, key, update }) => (
                      <Button key={key} variant={active ? 'default' : 'outline'} size="sm" className="flex-1 h-9"
                        onClick={() => editor.pushHistory(editor.annotations.map(a =>
                          a.id === editor.selectedAnnotationId ? { ...a, ...update } : a
                        ))}
                      >
                        {icon}
                      </Button>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="text-xs text-muted-foreground">{selectedAnnotation.opacity ?? 100}%</span>
                  </div>
                  <Slider
                    value={[selectedAnnotation.opacity ?? 100]}
                    min={10} max={100} step={1}
                    onValueChange={([v]) => editor.pushHistory(editor.annotations.map(a =>
                      a.id === editor.selectedAnnotationId ? { ...a, opacity: v } : a
                    ))}
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {PRESET_COLORS.map(c => (
                      <button
                        ...
                        style={{
                          backgroundColor: c,
                          borderColor: selectedAnnotation.color === c ? 'hsl(var(--primary))' : 'hsl(var(--border))',
                          boxShadow: selectedAnnotation.color === c ? '0 0 0 2px hsl(var(--primary)/0.3)' : 'none',
                        }}
                        onClick={() => editor.pushHistory(editor.annotations.map(a =>
                          a.id === editor.selectedAnnotationId ? { ...a, color: c } : a
                        ))}
                      />
                    ))}
                    <label className="w-7 h-7 rounded-full border-2 border-border overflow-hidden cursor-pointer hover:scale-110 transition-transform relative" title="Custom color">
                      <input
                        ...
                        onChange={e => editor.pushHistory(editor.annotations.map(a =>
                          a.id === editor.selectedAnnotationId ? { ...a, color: e.target.value } : a
                        ))}
          ...
          <TabsContent value="layers" className="mt-0">
            {editor.annotations.length === 0 ? (
              ...
              <div className="space-y-1">
                {[...editor.annotations].reverse().map(ann => (
                  <div
                    key={ann.id}
                    onClick={() => editor.setSelectedAnnotationId(ann.id)}
                    className={`flex items-center justify-between px-3 py-2 rounded-md cursor-pointer text-sm transition-colors ${editor.selectedAnnotationId === ann.id ? 'bg-primary/10 text-primary border border-primary/30' : 'bg-background border border-border hover:border-primary/30'}`}
